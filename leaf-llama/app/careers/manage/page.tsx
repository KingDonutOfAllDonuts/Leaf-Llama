'use client';
import { fetchAccountData, getAllOrders, updateOrderStatus } from '@/app/apiActions';
    ...
    <div className="w-full relative flex flex-col">
      ...
      <div className="flex flex-grow p-4 mt-[85px]">
        ...
        <div className="w-1/2 border rounded-lg p-4 shadow-md bg-white mr-4">
          ...
          {orders == null ? '':
          <div>
            <OrdersDisplay orders={orders.inProgress} completed={false} onChecked={inProgressToComplete}/>
          </div>
          }
        </div>
        ...
        <div className="w-1/2 border rounded-lg p-4 shadow-md bg-white">
          ...
          {orders == null ? '':
          <div>
            <OrdersDisplay orders={orders.complete} completed={true} onChecked={completeToInProgress}/>
          </div>
          }
        </div>
      </div>
...
const OrdersDisplay = ({orders, completed, onChecked}) => {
  ...
  if (orders == null || orders.length==0) {
    return completed ?  <p>No completed orders.</p> : <p>No orders in progress.</p>
  }
  console.log(orders)
  return (
    <div>
        ...
        >
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-semibold">
              {order.details.serviceType=='pickup'?'Pick Up':`Table Number: ${order.details.tableNumber}`} -{" "}

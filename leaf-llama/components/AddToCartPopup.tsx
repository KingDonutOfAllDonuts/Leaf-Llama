...
const AddToCartPopup = ({isOpen, foodData, closePopup, handleSubmit}) => {
  ...
  const validateAndSubmit = () => {
    ...
    const missingOptions = optionTypes.singular.filter(
      (option) =>
        foodData.customization[option] && !selectedOptions[option] // Check for missing singular options
    );
    ...
    <AnimatePresence>
      {isOpen && foodData!=null && (
        ...
        >
          {/* content of popup */}
          <motion.div
            className="bg-white rounded-lg shadow-lg w-[450px] h-[calc(100vh-100px)] relative"
          ...
          >
            ...
            <div className='w-full h-[calc(100%-40px)] overflow-y-auto p-5'>
              ...
              <div className='border-b-2 border-orange-500 pb-2'>
                ...
                <p className='text-sm text-gray-600 my-1'>{formatPrice(foodData.price)}</p>
              ...
              {foodData.customization &&
                Object.keys(foodData.customization).map((optionName) => {
                  const isMultiple = optionTypes.multiple.includes(optionName);

                  return (  
                    <div key={optionName} 
                    className={`space-y-2 border-b border-gray-400 p-2 pb-4 mb-1 ${
                      incompleteOptions.includes(optionName) ? "border border-red-500 rounded p-2" : ""
                    }`}
                    ref={(el) => (optionRefs.current[optionName] = el) as any}
                          ...
                          <input
                            ...
                            checked={
                              isMultiple
                                ...
                                : selectedOptions[optionName] === value || false
                            }
                            onChange={() =>
                              handleOptionChange(optionName, value, isMultiple)
                            }

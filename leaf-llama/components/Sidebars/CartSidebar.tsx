        ...
        <div className="p-4 h-[calc(100vh-128px)] overflow-y-auto">
          {cart.length <= 0 ? (
            ...
            cart.map((cartFood, i) => {
              ...
              return (
                <div 
                onClick={() => {toggleCartSidebar(); openPopup(cartFood.data)}}
                className='flex gap-2 border-b-2 py-3 px-2 cursor-pointer transition-all hover:bg-slate-100 hover:opacity-80' key={i}>
                  <p className='text-xl text-gray-500'>{cartFood.quantity}</p>
                  <div className='flex flex-col w-full'>
                    <div className='flex justify-between w-full'>
                      <h3 className='font-medium text-black text-xl'>{cartFood.data.name}</h3>
                      <div className='flex'>
                        <h4 className='text-gray-500 text-sm'>{formatPrice(cartFood.data.price*cartFood.quantity)}</h4>
                        <FaTrashAlt className='w-4 h-4 ml-3 text-red-700 hover:text-red-300 transition-all'
                          onClick={(e) => {
